...
class FlashcardDetails(APIView):
    def get_object(self, collection, pk):
        try:
            return Flashcard.objects.get(collection=collection, pk=pk)
        except Flashcard.DoesNotExist:
            raise Http404

    def get(self, request, collection, pk):
        flashcard = self.get_object(collection, pk)
        serializer = FlashcardSerializer(flashcard)
        return Response(serializer.data)

    def put(self, request, collection, pk):
        flashcard = self.get_object(collection, pk)
    ...
    def delete(self, request, collection, pk):
        flashcard = self.get_object(collection, pk)
        serializer = FlashcardSerializer(flashcard)
        flashcard.delete()
